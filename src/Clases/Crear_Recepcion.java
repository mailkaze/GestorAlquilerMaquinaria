 ...
 * @author Kaze
 */
public class Crear_Recepcion {
    public static void crearRecepcion(String NomYApe,String Fecha, int index1,int index2,int index3,int index4,int index5,int index6) throws IOException{
        //Conectamos con la base de datos
            ...
            POIFSFileSystem fs2 = new POIFSFileSystem(new FileInputStream("docs/PlantillaRecepción.xls"));
...
//            CeldaD23.setCellValue(Clases.Clientes.CCI);
            ...
            try {
                FileOutputStream elFichero = new FileOutputStream("docs/Recepción-"+Clases.Alquileres.ID_Alquiler+".xls");
                plantilla.write(elFichero);
                elFichero.close();
            } catch (Exception e) {
                e.printStackTrace();
                JOptionPane.showMessageDialog(null, "Ocurrió un error al crear la hoja de Recepción.");
            }
            
            Desktop desktop = Desktop.getDesktop(); //Instanciamos la clase Desktop
            String ruta="docs/Recepción-"+Clases.Alquileres.ID_Alquiler+".xls"; //Le damos la ruta del archivo.
